...
| `pkg/builder/backup_builder.go:107` | `ScheduleNameLabel` | `schedule.Name` | `label.GetValidNameLongHash(schedule.Name)` |
...
The `ScheduleNameLabel` row uses `label.GetValidNameLongHash` rather than `label.GetValidName` — see Category E.2 and "Truncate without a hash suffix" under Alternatives Considered for why: unlike the other four rows here, `ScheduleNameLabel` is not merely selector-only, it is also read by several places that assume an exact match (E.2), and today's code never successfully hashes it at all (a long schedule name just fails outright), so there is no existing 6-character-hashed value anywhere to stay compatible with. That makes it safe to give it the stronger, 16-character hash from day one, the same way `GetValidObjectName` does for brand-new functions.
...
| --- | --- | --- | --- |
| `pkg/cmd/cli/restore/create.go:251` | `velero restore create --from-schedule <name>` | `labels.SelectorFromSet(map[string]string{api.ScheduleNameLabel: o.ScheduleName})` | `labels.SelectorFromSet(map[string]string{api.ScheduleNameLabel: label.GetValidNameLongHash(o.ScheduleName)})` |
| `pkg/cmd/cli/restore/create.go:313` | `--allow-partially-failed` lookup for the same restore | `labels.SelectorFromSet(map[string]string{api.ScheduleNameLabel: o.ScheduleName})` | `labels.SelectorFromSet(map[string]string{api.ScheduleNameLabel: label.GetValidNameLongHash(o.ScheduleName)})` |
| `pkg/controller/restore_controller.go:388` | Fill in `BackupName` from the most recent backup of `restore.Spec.ScheduleName` | `labels.SelectorFromSet(labels.Set(map[string]string{api.ScheduleNameLabel: restore.Spec.ScheduleName}))` | `labels.SelectorFromSet(labels.Set(map[string]string{api.ScheduleNameLabel: label.GetValidNameLongHash(restore.Spec.ScheduleName)}))` |
| `pkg/controller/schedule_controller.go:234` | `checkIfBackupInNewOrProgress`, used to skip a new backup while one is already running | `labels.Set(map[string]string{velerov1.ScheduleNameLabel: schedule.Name}).AsSelector()` | `labels.Set(map[string]string{velerov1.ScheduleNameLabel: label.GetValidNameLongHash(schedule.Name)}).AsSelector()` |
...
// length alone and is treated as a possible hash, so it is not persisted.
if restore.Spec.ScheduleName == "" {
...
**A tempting-but-wrong refinement, rejected in review.** An earlier version of this fix tried to resolve the exactly-63-character ambiguity instead of merely accepting it: since `Schedule` is an independently-gettable object with its own real, never-hashed `metadata.name`, a `Get` for a `Schedule` named exactly `scheduleName` seemed like it would settle the question — a match would prove the label was never hashed, since `GetValidNameLongHash` is the identity function below 64 characters.

This does not actually hold. A `Get` match only proves that *some* `Schedule` with that exact name exists in the namespace — not that it's the one this restore's backup actually came from. If the real originating schedule's name is longer than 63 characters (the case this whole fix exists for) and its hash happens to exactly equal the name of a *different*, unrelated `Schedule` in the same namespace, the `Get` succeeds, and the refinement would confidently persist that unrelated schedule's name into `restore.Spec.ScheduleName` — a wrong value asserted with false confidence, which is worse than the honest "empty" the simple length check produces. This isn't a 64-bit hash-collision scenario (two long names hashing to the same value): it only requires one ordinary, human-chosen `Schedule` name to coincidentally match a computed hash string, which — while still unlikely for organically-named schedules — is far more plausible than an actual hash collision, and becomes materially more likely in environments that create many schedules from templated or programmatically-generated names sharing a common prefix. Resolving it properly would need an authoritative identifier (e.g. a UID) tying the restore's backup to a specific schedule, not just a name match; that machinery doesn't exist today and adding it is the same disproportionate-for-a-metrics-field annotation mechanism already ruled out elsewhere in this design. The simple length check has no such failure mode — its only cost is a false *negative* (an honest empty value for a rare, legitimate 63-character schedule name), not a false *positive* (a confidently wrong one), so it is kept as the actual fix.

The trade-off: `restore.Spec.ScheduleName` is left empty for schedule names that exceed 63 characters (the case Category D.1 addresses) and, as a rare false negative, for the small number of genuinely-63-character schedule names too. Either way, the downstream metrics that read this field (`pkg/controller/restore_finalizer_controller.go:258`/`261`, and this same function's own `RegisterRestoreAttempt` call) report an empty schedule name rather than either the real name or a hash — strictly better than reporting a hash (or, worse, an unrelated schedule's real name) that looks like real data but isn't, and limited to the same narrow long-schedule-name case everything else in Category E.2 is already about.
...
The five Category D.1 fixes change label value assignment from a raw name to a hashing helper: `label.GetValidName(name)` for four of the five, and `label.GetValidNameLongHash(schedule.Name)` for `ScheduleNameLabel` (see Category D.1's own note on why that one differs).
For names ≤ 63 characters, both helpers return the input unchanged.
...
E.2's fifth fix, `restore_controller.go`'s `restore.Spec.ScheduleName` auto-fill, is compatibility-relevant in its own right: for restores auto-detected from a schedule with a name of 63 characters or more, `Spec.ScheduleName` changes from "populated with a hash for names over 63 characters" (the bug this fixes) to "left empty for names 63 characters or more" (accepting a rare false negative for a genuinely-63-character schedule name, rather than risk a wrong value — see Category D.1/E.2's note on why a smarter fix here was tried and rejected). Any existing tooling reading this field for restores from a >63-character-name schedule was already getting a hash before this design shipped Category D.1 at all — there is no prior "correct" value to regress from, since Category D.1 and this fix land in the same change.
...
**A second, distinct collision mode for `GetValidNameLongHash`/`GetValidName`: raw values and hashed values share the same output space.** For a schedule name ≤ 63 characters, `GetValidNameLongHash` returns it unchanged; for one > 63 characters, it returns a hash that is *always exactly* 63 characters. So a genuinely-existing schedule whose name happens to be exactly 63 characters produces a `ScheduleNameLabel` value that lives in the *same* 63-character space a hash could also occupy — unlike the "two long names collide with each other" risk discussed above, this is "a short (well, exactly-63-character) name coincidentally equals the hash of an unrelated long name." If that coincidence occurs, Category E.2's selectors would match backups from *both* schedules for a `--from-schedule` lookup on either one. This is not fixable by choosing a different hash length: any fixed-length hash output necessarily overlaps with the set of raw names of that same length, since Kubernetes doesn't reserve any character pattern that ordinary label values can't also take. Properly disambiguating would need an out-of-band identifier (e.g. resolving the `Schedule`'s UID rather than matching on name/label alone) — the same disproportionate-for-this-use-case machinery already declined above and in Category D.2/E.2. Given the coincidence requires a human-chosen, exactly-63-character schedule name to exactly match 16 pseudorandom-looking hex characters appended to a truncated prefix of a *different*, unrelated, longer schedule name, this is accepted as a residual risk in the same spirit as the 24-bit/64-bit collision bounds already documented above, rather than solved with new machinery; noted in Open Issues.
...
- **`ScheduleNameLabel`'s raw-value/hashed-value representation overlap at exactly 63 characters**: see "Truncate without a hash suffix" under Alternatives Considered. A schedule name that happens to be exactly 63 characters produces the same kind of label value a hash of a different, longer schedule name could also produce, so Category E.2's selectors could in principle match backups from both. Accepted as a residual risk (requiring a human-chosen name to coincidentally match 16 pseudorandom hex characters) rather than solved with an out-of-band identifier (e.g. resolving via `Schedule` UID), which is the same disproportionate-for-this-use-case machinery already declined elsewhere in this design; revisit only if it proves to matter in practice.
