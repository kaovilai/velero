...
- conditions:
  ...
  action:
    type: skip`,
		...
		{
			name: "PVC phase Pending should be skipped",
			yamlData: `version: v1
volumePolicies:
- conditions:
    pvc:
  ...
  action:
    type: skip`,
		...
		{
			name: "PVC phase Bound should not be skipped",
			yamlData: `version: v1
volumePolicies:
- conditions:
    pvc:
  ...
  action:
    type: skip`,
		...
		{
			name: "PVC phase Pending with bound PV should be skipped",
			yamlData: `version: v1
volumePolicies:
- conditions:
    pvc:
...
func TestParsePVC(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid PVC with empty labels and Bound phase",
			pvc: &corev1api.PersistentVolumeClaim{
				ObjectMeta: metav1.ObjectMeta{
